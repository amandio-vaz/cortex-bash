...
const CopyButton: React.FC<{ code: string }> = ({ code }) => {
  ...
  return (
    <Tooltip text={isCopied ? t('tooltipCopied') : t('tooltipCopy')}>
      <button
        onClick={handleCopy}
        className="bg-slate-700/50 hover:bg-slate-600/50 text-gray-300 text-xs px-2 py-1 rounded-md transition-all flex items-center"
      >
        {isCopied ? (
...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ title, content, isLoading, isThinking, isFullscreen, onToggleFullscreen }) => {
  ...
  const renderContent = () => {
    if (isLoading && isThinking) {
      return (
        <div className="flex flex-col justify-center items-center h-full text-center text-gray-400">
          ...
          <p className="mt-4 text-lg font-semibold text-gray-300">{t('thinkingMessage')}</p>
    ...
    const parts = content.split(/(```bash[\s\S]*?```|```[\s\S]*?```)/g);

    return parts.map((part, index) => {
        if (part.startsWith('```')) {
            const code = part.replace(/```bash\n|```\n|```/g, '');
            return (
                <div key={index} className="bg-slate-900/70 rounded-lg my-4 border border-white/10 overflow-hidden">
                    <div className="flex justify-between items-center px-4 py-1.5 bg-black/20 border-b border-white/10">
                        <span className="text-xs font-mono text-cyan-400">bash</span>
        ...
        return <div key={index} dangerouslySetInnerHTML={{ __html: part.replace(/`([^`]+)`/g, '<code class="bg-slate-700/50 text-cyan-400 py-0.5 px-1.5 rounded text-sm font-mono">$1</code>').replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') }} className="whitespace-pre-wrap my-2 text-gray-300" />;
    });
  };
  ...
  return (
    <div className="bg-black/20 backdrop-blur-xl border border-white/10 rounded-xl flex flex-col h-full p-4 shadow-2xl shadow-black/20">
      ...
      <div className="prose prose-invert max-w-none flex-grow overflow-y-auto pr-2 text-gray-300">
        {renderContent()}
      </div>
    </div>
  );
